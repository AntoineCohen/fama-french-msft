...
#let make-authors(authors) = par({
  ...
  authors.map(make-author).join(", ", last: " et ")
})
  ...
  if author.at("corresponding", default: false) == true {
    place(
      ...
      bottom,
      {
        ...
        if els-columns == 1 {
           [#h(1em);#super[#sym.ast]#h(0.1em);Auteur correspondant. Adresse e-mail : #if (author.at("email", default: none)) != none {author.email} else {"Adresse non renseignée"}]
        } else {
            [#h(1em);#super[#sym.ast]#h(0.1em);Auteur correspondant. #linebreak() #h(1.4em);Adresse e-mail : #if (author.at("email", default: none)) != none {author.email} else {"Adresse non renseignée"}]
        }
      }
    )
  }
